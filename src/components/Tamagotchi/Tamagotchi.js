...
class Tamagotchi extends React.Component {
    ...
    reduceStats = stat => {
        if (stat === 'life') {
            ...
            if (fun + health + hunger < 100) {
                newLife = this.state[stat] - 10;
            } else if (fun + health + hunger < 150) {
                newLife = this.state[stat] - 5;
            } else if (fun + health + hunger < 200) {
                newLife = this.state[stat] - 2.5;
            } else {
                newLife = this.state[stat] < 96 ? this.state[stat] + 5 : 100;
            }
            this.setState({
                [stat]: newLife
            });
        ...
        return (
            <div>
                ...
                <StatBar
                    ...
                    image={lifeIcon}
                    increase={() =>
                        this.state.life < 100 ? this.increaseLife() : null
                    }
                />
            </div>
        );
